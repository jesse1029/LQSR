...
nwin_y=3;
B=72;%set here the number of histogram bins
[L,C]=size(Im); % L num of lines ; C num of columns
numberhog=8;
H=zeros(nwin_x*nwin_y*numberhog,1); % column vector with zeros
m=sqrt(L/2);
...
for n=0:nwin_y-1
    for m=0:nwin_x-1
        ...
        H2=H2/(norm(H2)+0.01);
        H3=zeros(numberhog,1);
        bin=1;
        for k=1:72
            if k==18 || k==19 || k==54 || k==55  
                H3(bin)=H2(k);
                bin=bin+1;
            end
        end      
        H((cont-1)*numberhog+1:cont*numberhog,1)=H3;
    end
end